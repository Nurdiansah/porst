...
if ($Area == 'kj4') {
    $query = mysqli_query($koneksi, "SELECT * FROM stacking s
                                                ...
                                                WHERE status_stacking = '3' AND area = 'kj4'
                                                ");

    $queryReject = mysqli_query($koneksi, "SELECT * FROM stacking s
                                                JOIN alat_berat a
                                                ON a.id_alatberat = s.id_alatberat 
                                                JOIN client c
                                                ON c.id_client = s.id_client
                                                WHERE status_stacking = '20' AND area = 'kj4'
                                                ");
} else {
    $query = mysqli_query($koneksi, "SELECT * FROM stacking s
                                                JOIN alat_berat a
        ...
        <div class="col-sm-12">
            <div class="box box-primary">
                <div class="box-header with-border">
                    <h3 class="text-center">Stacking/Stuffing</h3>
                </div>
                <div class="box-body">
                                ...
                                <tbody>
                                    <tr>
                                        ...
                                        ?>
                                                ...
                                                <td>
                                                    <a href="?p=stf_dstacking&id=<?= base64_encode($row['id_stacking']); ?>"><button class="btn btn-info"> Detail </button></a>
                                                </td>
                                    </tr>
                            <?php $no++;
                                            endwhile;
                                ...
                                <table class="table text-center table-striped table-hover">
                                    ...
                                    <tbody>
                                        <tr>
                                            ...
                                            ?>
                                                    <td> <?= $no; ?> </td>
                                                    <td> <?= $rowR['id_stacking']; ?> </td>
                                                    <td><?= tanggal($rowR['created_on']); ?> </td>
                                                    <td> <?= $rowR['nm_alatberat']; ?> </td>
                                                    <td> <?= $rowR['nm_client']; ?> </td>
                                                    <td> <?= $rowR['nm_kegiatan']; ?> </td>
                                                    <td> <?= limitKata($rowR['komentar']); ?> </td>
                                                    <td>
                                                        <a href="?p=stf_dstacking&id=<?= base64_encode($rowR['id_stacking']); ?>"><button class="btn btn-danger"> Detail </button></a>
                                                    </td>
                                        </tr>
                                <?php $no++;
                                                endwhile;
