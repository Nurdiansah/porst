...
<!-- Setting Margin header/ kop -->
  ...
  $sql = mysqli_query($koneksi, "SELECT * FROM job_order j
                                          JOIN client c
                                          ON c.id_client = j.id_client
  ...
  <table class="tabel2 text-center" align="center"  >
    ...
    <tbody>
      ...
      <?php    
        $queryts = mysqli_query($koneksi, "SELECT ac.time_as, ac.doc_no_as, sum(ac.qty_as) as qty_as, ac.rincian_cargo, ac.remarks_as, ac.keterangan, sum(ac.m3_as) as m3_as, sum(ac.ton_as) as ton_as, sum(ac.revton_as) as revton_as, d.id_cargo
                                            FROM actual_stevedoring ac
                                            JOIN detail_joborder d        
                                            ON ac.doc_no_as = d.doc_no
                                            WHERE ac.id_joborder = '$idJoborder'
                                            AND ac.keterangan != 'Not Available'
                                            GROUP BY ac.doc_no_as
                                            ORDER BY d.id_cargo 
      ...
      <tr>
        ...
        <td style="text-align: center; width=50px;"><?php echo $row['doc_no_as']; ?></td>
