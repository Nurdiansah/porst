...
if (isset($_POST['simpan'])) {
    ...
    $query = "INSERT INTO pemakaian_listrik (lokasi, bulan, meter_awal, tanggal_awal, meter_akhir, tanggal_akhir, jml_pemakaian, faktor, total_meter, beban_puncak, satuan_kwh, tagihan_pln) VALUES 
                                    ('$lokasi', '$bulan', '$meter_awal', '$tanggal_awal', '$meter_akhir', '$tanggal_akhir', '$jml_pemakaian', '$faktor', '$total_meter', '$beban_puncak', '$satuan_kwh', '$tagihan_pln');
                ";

    $hasil = mysqli_query($koneksi, $query);
    if ($hasil) {
        header("location:index.php?p=add_pemakaianlistrik");
    } else {
...
<script>
    ...
    function kurang_meter() {
        ...
        if (!isNaN(result)) {
            document.getElementById('jml_pemakaian').value = result.toFixed(3);
        }
    }
    ...
    function kali_faktor() {
        ...
        var result = parseFloat(txtFirstNumberValue) * parseFloat(txtSecondNumberValue);
        if (!isNaN(result)) {
            document.getElementById('total_meter').value = result.toFixed(3);
        }
    }
    ...
    function sum_kwh() {
        ...
        var result = parseFloat(txtFirstNumberValue) + parseFloat(txtSecondNumberValue);
        if (!isNaN(result)) {
            document.getElementById('satuan_kwh').value = result.toFixed(3);
        }
    }
</script>